...
public class add_client extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        if(v.getId() == registrar.getId()){
                ...
                public void run() {
                    try {
                        synchronized (this) {
                            wait(500);
                            conectarBDMySQL();
                            addCliente(id,txt_nom,txt_mote,txt_tlf, latitud,longitud,tenda.getId());
                            cerrarConexion();
                        }
            ...
            if(bd.getResult() == 0){
                Toast.makeText(this, "El cliente " + id + " con nombre " + txt_nom + " ha sido registrado en tu tienda", Toast.LENGTH_LONG).show();
    ...
    protected void onDestroy() { //Utilizado para que al destruirse vaya al dashboard directamente
        Intent intent = new Intent(this,DashBoard.class).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        startActivity(intent);
        super.onDestroy();
    }
}
